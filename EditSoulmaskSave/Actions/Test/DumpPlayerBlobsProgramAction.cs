...
{
	/// <summary>
	/// Dumps all player state actor data blobs in a save file to individual binary files
	/// </summary>
	internal class DumpPlayerBlobsProgramAction : ProgramAction
	{
		public string SavePath { get; private set; }

		public string OutDir { get; private set; }

		public DumpPlayerBlobsProgramAction(string savePath, string outDir)
		{
			SavePath = savePath;
			OutDir = outDir;
		}
		...
		{
			logger.Important("Dumping player state actor raw data...");

			bool success = true;
			foreach (SaveDataRow row in ActorDataUtil.GetActors(SavePath, "/Script/WS.HPlayerState", logger))
			{
				string outPath = Path.Combine(OutDir, $"{row.Name}.dat");
